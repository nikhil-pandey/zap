...
async def test_repo_explorer_complex_structure(complex_nested_repo):
    ...
    for dep in expected_dependencies:
        assert any(
            dep in dep_info.dependencies for dep_info in dependencies.values()
        ), f"{dep} not found in dependencies"

    # Check if the structure is correctly analyzed
    assert len(result.file_change_count) >= len(
        expected_files
    ), "Not all expected files were analyzed"
