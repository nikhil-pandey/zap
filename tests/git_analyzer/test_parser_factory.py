...
from zap.git_analyzer.models.enums import DependencyFileType
...
def test_parser_factory_unsupported_file():
    with pytest.raises(ValueError, match="No parser available for .*"):
        ParserFactory.get_parser("unsupported_file_type.txt")
...
def test_parser_factory_is_dependency_file():
    ...
    assert not ParserFactory.is_dependency_file("some_random_file.txt")
