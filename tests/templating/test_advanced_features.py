...
async def test_include_valid_url(mocker):
    template_engine = ZapTemplateEngine(templates={'index.html': '{{ i("https://valid.url") }}'})

    # Mock aiohttp.ClientSession().get() to return valid content for a valid URL
    mock_response = mocker.AsyncMock()
    mock_response.__aenter__.return_value.text = mocker.AsyncMock(return_value='Valid content')
...
async def test_include_slow_response_url(mocker):
    template_engine = ZapTemplateEngine(templates={'index.html': '{{ i("https://slow.url") }}'})
